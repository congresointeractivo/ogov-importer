    ...
    initEnv: function (url, callback) {
      ...
      var doFetch = function () {
        request({url: url, encoding:"binary"}, function (err, response, body) {
          ...
          if (body) {
            var enc = jschardet.detect(body).encoding.toLowerCase();

            if (enc !== 'utf-8') {
              if (!iconv[enc]) {
                iconv[enc] = new Iconv(enc, 'UTF-8//TRANSLIT//IGNORE');
              }
              body = iconv[enc].convert(new Buffer(body, 'binary')).toString('utf-8');
            }
