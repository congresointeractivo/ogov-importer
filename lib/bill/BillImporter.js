...
module.exports = function BillImporter(options) {
  ...
  var DATA_SOURCE = "http://www1.hcdn.gov.ar/proyectos_search/resultado.asp?" +
    ...
    "&fecha_inicio=${startDate}" +  // Bills start date.
  ...
  var processPage = function (pageNumber, callback) {
    ...
    processDate = process.argv[3];
    if (processDate) {
      url = url.replace("${endDate}", processDate)
        .replace("${startDate}", processDate);

    }
    else {
      var now = new Date();
      url = url.replace("${endDate}", now.getDate() + "/" + (now.getMonth() + 1) + "/" +
        now.getFullYear())
        .replace("${startDate}", "01/01/1999")
    ...
    importer.initEnv(url, function (errors, $) {
      if ($ === undefined) {
        LOG.error("BillImporter::processPage ",errors);
        return callback(new Error("BillImporter::processPage:" + pageNumber));
      }
      var documents = $(".toc");
  ...
  var extractProcedures = function (context, rawBill, callback) {
    var proceduresRows = context.find("div.item1 tr:contains(TRAMITE)+tr");
