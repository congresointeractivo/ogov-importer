...
module.exports = function PopoloStorer(instance, options) {
  ...
  function toPopolo(ogovBill) {
    ...
    for (d in ogovBill.dictums) {
      ...
      var billitPaperwork =  {}
      if (!dictum["date"]) {
        console.log("Error: Dictum without date",ogovBill.file,dictum)
      }
      else {
    ...
    for (p in ogovBill.procedures) {
      ...
      if (!procedure["topic"]) {
        console.log("Error: procedure without topic",procedure,ogovBill.file)
      } else {
        var billitDirective = {};
        ...
        billitDirective.bill_id = procedure["file"]
    ...
    if (!ogovBill.summary) {
      console.log("NO SUMMARRY",ogovBill);
    }
  ...
  function pushToBillit(instance,data) {
   ...
   request(
    ...
    , function (error, response, body) {
      var method;
        if(!response){
          console.log("Connection error", url, data.uid);
          return;
        }
        else if(response.statusCode == 500){
          console.log("Server error", data.uid);
          return;
        }
        ...
        , function (error, response, body) {
            if(response && response.statusCode == 302){
              console.log('document saved',data.uid,method,response.statusCode);
            } else {
              console.log('error: ' + url + ' ' + response.statusCode || response, data.uid)
              console.log(body)
            }
          }
        )
  ...
  return {
    ...
    store: function (id, data, role, callback) {
      ...
      process.nextTick(function () {
        popoloData = toPopolo(data);
        pushToBillit(instance,popoloData);
