...
module.exports = function PopoloStorer(instance, options) {
  ...
  */
  ...
  /** votesToPopolo
   * @type Function
   * @param ogovVote ogovVote
   * Recieves an vote formatted by voteImporter and exports a popolo object
   */
  function votesToPopolo(ogovVote) {

    //console.log("ogovVote",ogovVote);

    //El problema es que a veces el PDF no se parsea bien
    //Y corta, en vez de importar todos los votos, sólo lo hace con algunos

    var billitVotes = []

    for (v in ogovVote.votes) {
      var vote = ogovVote.votes[v];
      billitVotes.push({
        voter_id: vote.name, 
        option: vote.vote
      });
    ...
    if (ogovVote.summary.length > 1) {
      billitCount = [
        { "option": "SI", value: ogovVote.summary.affirmative.members },      
        { "option": "NO", value: ogovVote.summary.negative.members },
        { "option": "ABSTENCION", value: ogovVote.summary.abstention.members },
        { "option": "AUSENTE", value: ogovVote.summary.absent }
      ];
    ...
    var billitMotion = [
      { 
        organization: "Diputados",
        text: ogovVote.references.summary,
        date: ogovVote.date,
        requirement: ogovVote.summary.majorityType,
        vote_events: [
          {
            counts: billitCount,
            votes: billitVotes
   ...
   */
  function billsToPopolo(ogovBill) {
    ...
    billitBill.paperworks.forEach(function (billitPaperwork) {
      if(billitPaperwork.chamber === -1) {
        if (billitPaperwork.chamber === billitBill.source) {
          billitBill.stage = BILL_STAGE.DICTUM_ORIGIN;
        }
        else {
          billitBill.stage = BILL_STAGE.DICTUM_REVISORY;
        }
      }
    });
    ...
    billitBill.directives.forEach(function(billitDirective) {
      if (billitDirective.chamber === billitBill.source) {
        ...
        var approved_stages = ["SANCIONADO", "APROBADO", "ARCHIVADO"];
        ...
        if (billitDirective.step.indexOf("CONSIDERACION") > -1 ||
          ...
          if (billitDirective.stage === "MEDIA SANCION") {
            billitBill.stage = BILL_STAGE.HALF_SANCTION;
          ...
          else if (approved_stages.indexOf(billitDirective.stage) > -1) {
            billitBill.stage =  BILL_STAGE.APPROVED;
          }
          else if (billitDirective.stage === "RECHAZADO") {
            billitBill.stage = BILL_STAGE.REJECTED;
          }
          else if (billitDirective.stage === "RETIRADO") {
            billitBill.stage = BILL_STAGE.REJECTED;
          }
          else if (billitDirective.stage === "") {
            billitBill.stage =  BILL_STAGE.CONSIDERING;
          }
          else {
    ...
    if(creation_year + project_duration < new Date().getYear()) {
      billitBill.stage = BILL_STAGE.PARLIAMENTARY_STATUS_LOST;      
    }
   ...
   * @param {Object} billitObject JSON Structure for a bill or vote, formatted using the Popolo Standard
   * @param {function} callback Function for marking the queue process end
   */
  function pushToBillit(billitUrl,billitObject,callback) {
   ...
   request(
      { 
        method: 'GET', 
        uri: url + "/" + billitObject.uid + ".json?fields=uid"
      }, 
      function (error, response, body) {
        var method,uri;
        if(!response){
          LOG.error("Connection error", url, billitObject.uid);
          billitObject = null;
          callback();
          return;
        }
        else if(response.statusCode === 500){
          LOG.error("Server error", billitObject.uid);
          billitObject = null;
          callback();
          return;
        }
          ...
          method = "PUT";
          uri = url + "/" + billitObject.uid;
        } else if (billitObject.subject_areas) {
          method = "POST";
          uri = url;
        }
        else {
          LOG.error("Attempting to push new bill without subject areas",billitObject);
          return;
        }
        ...
        request(
          { 
            ...
            body: JSON.stringify(billitObject)
          }, 
          function (error, response, body) {
            if(response && response.statusCode === 302){
              LOG.info('document saved',billitObject.uid,method,response.statusCode);
            } else {
              LOG.error('error: ' + url + ' ' + response.statusCode || response, billitObject.uid);
              LOG.error(body);
            }
            billitObject = null;
            callback();
  ...
  return {
    ...
    store: function (id, data, role, callback) {
      switch (role) {
        case "bills":
          queue.push(billsToPopolo(data));
          break;
        case "vote":
          queue.push(votesToPopolo(data));
          break;
        default:
          console.log("PopoloStorer not implemented for "+role);
          break;
      }
      callback();
    },
