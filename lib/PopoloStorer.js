  ...
  */
  ...
  function toPopolo(ogovBill) {
    ...
    for (d in ogovBill.dictums) {
      ...
      var billitPaperwork =  {}
      if (!dictum["date"]) {
        //Estoy asumiendo que todos los proyectos aprobados por 114 o 204 que no tengan
        //fecha, fueron aprobados en la fecha de ingreso
        //Reglamento: http://www1.hcdn.gov.ar/dependencias/dip/congreso/regladip.pdf
        if (dictum["result"].indexOf("ARTICULO 114") > -1 ||
          dictum["result"].indexOf("ARTICULO 204") > -1) {
          dictum["description"] = dictum["result"];
          dictum["result"] = "APROBADO";
...
//    }
  ...
  function pushToBillit(instance,data,callback) {
    ...
    // });
    //if (data.paperworks.length == 0) { delete data; callback(); return; }

   request(
      { method: 'GET'
      , uri: url + "/" + data.uid + ".json?fields=uid"
      }
